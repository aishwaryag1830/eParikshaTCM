...
function padout(number) { return (number < 10) ? '0' + number : number;}//padding for length less than 2 digit
...
function newCalendar() 
{
	...
	for (var intWeek = 0; intWeek < tableCal.rows.length;  intWeek++)
			 ...
			 intDay++)
	 {
		  ...
		  cell.style.color = (day == daily) ? "#A83B3E" : "";
		  if(day == daily)
		  {
				document.all.todayday.innerText= "Today: " + padout(newCal.getMonth()+1) + "/" + padout(day)+ "/" + newCal.getFullYear() ;
		  }
	 ...
	 {
			    document.all[element_id].value = padout( padout(today.month+1)+ "/" + today.day)  + "/" + today.year; 
        ...
        function getDate() 
		 {
            ...
            if ("TD" == event.srcElement.tagName)
			   ...
			   { 
			    //String d =event.srcElement.innerText;
			  // if(d.Length()==1)
			   //d="0"+d;
				 var mn = document.all.month.selectedIndex+1;
				 	 
    			 var Year = document.all.year [document.all.year.selectedIndex].text;
				 document.all[element_id].value= padout(mn) +"/"+ padout(event.srcElement.innerText)+"/"  +Year;
		         //document.all.calendar.style.visibility="hidden";
				 document.all.calendar.style.display="none";
			 }
		 }
