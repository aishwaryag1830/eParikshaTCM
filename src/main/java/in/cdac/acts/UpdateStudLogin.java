...
public class UpdateStudLogin extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (session != null && !session.equals("") && request.getParameter("prn") != null
				&& request.getParameter("flag") != null && session.getAttribute("CourseId") != null
				&& !session.getAttribute("CourseId").equals("")) {
			String strStudPRN = request.getParameter("prn");
			String strLoginFlag = request.getParameter("flag");
			String strQuery = null;
			if (strStudPRN.equalsIgnoreCase("999")) {
				strQuery = "update ePariksha_Student_Login set stud_Login_Flag=? where stud_Course_Id=?";
			} else {
				strQuery = "update ePariksha_Student_Login set stud_Login_Flag=? where stud_Course_Id=? and stud_PRN=TRIM(?)";
			}
			...
			try {
				...
				statement.setBoolean(1, Boolean.parseBoolean(strLoginFlag));
				...
				if (iRowAffected > 0) {
					strStudentLoginMsg.append(
							"<font style='color:green;' class='schedule_msg'> Login has been reset successfully.</font>");
				} else {
					strStudentLoginMsg.append(
